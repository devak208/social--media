...
class DefaultMessage extends message_1.default {
    ...
    static fromValues(values, options) {
        return (0, message_1.fromValues)(values, options);
    }
    // Used by tests
...
const MessageActionMap = new Map(MessageActionArray.map((action, index) => [action, index]));
const ReverseMessageActionMap = new Map(MessageActionArray.map((action, index) => [index, action]));
...
async function fromEncoded(logger, Crypto, encoded, inputOptions) {
    const msg = fromValues(encoded, { stringifyAction: true });
...
function fromValues(values, options) {
    const stringifyAction = options === null || options === void 0 ? void 0 : options.stringifyAction;
    if (stringifyAction) {
        const action = toMessageActionString(values.action) || values.action;
        return Object.assign(new Message(), Object.assign(Object.assign({}, values), { action }));
    }
...
class Message {
    ...
    toString() {
        ...
        if (this.updateSerial)
            result += '; updateSerial=' + this.updateSerial;
        if (this.operation)
...
function fromDeserialized(deserialized, presenceMessagePlugin) {
    ...
    if (messages) {
        for (let i = 0; i < messages.length; i++) {
            messages[i] = (0, message_1.fromValues)(messages[i], { stringifyAction: true });
        }
    }
