...
var MessageActionArray = ["message.unset", "message.create", "message.update", "message.delete", "annotation.create", "annotation.delete", "meta.occupancy"];
var MessageActionMap = new Map(MessageActionArray.map((action, index) => [action, index]));
var ReverseMessageActionMap = new Map(MessageActionArray.map((action, index) => [index, action]));
...
function fromValues(values, options) {
  const stringifyAction = options == null ? void 0 : options.stringifyAction;
  if (stringifyAction) {
    const action = toMessageActionString(values.action) || values.action;
    return Object.assign(new Message(), __spreadProps(__spreadValues({}, values), {
      action
    }));
  }
